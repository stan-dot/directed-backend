...
from app.utils import generate_pseudonym
...
def create_student(student: schemas.StudentCreate, db: Session = Depends(get_db)):
    ...
    if old_student:
        raise HTTPException(
            ...
            detail=f"student with id: {student.personal_id} already exist, cannot create a new one with the same name.",
        )
...
def get_students(
    ...
    students = (
        ...
        .filter(models.Students.name.contains(search))
        .limit(limit)
        .offset(offset)
        .all()
    )
...
@router.get("/{personal_id}", response_model=schemas.Student)
...
@router.put("/{personal_id}", response_model=schemas.Student)
def update_student(
    personal_id: str,
    updated_student: schemas.StudentCreate,
    db: Session = Depends(get_db),
):
    student_query = db.query(models.Students).filter(
        models.Students.personal_id == personal_id
    )
...
def get_students_from_cohort_school(
    cohort_name: str, school_name: str, db: Session = Depends(get_db)
):
    """
    ...
    students = (
        db.query(models.Students)
        .filter(
            models.Students.cohort == cohort_name, models.Students.school == school_name
        )
        .order_by(models.Students.name)
        .all()
    )
    return students
...
@router.put("/{personal_id}/milestone", response_model=schemas.Student)
def update_student_milestone(personal_id: str, db: Session = Depends(get_db)):
    ...
    student_query.update(
        {"milestones_achieved": student.milestones_achieved + 1},
...
@router.put("/{personal_id}/send_grant", response_model=schemas.Student)
def update_student_milestone(
    personal_id: str, new_grant_received: int, db: Session = Depends(get_db)
):
    student_query = db.query(models.Students).filter(
        models.Students.personal_id == personal_id
    )
    ...
    student_query.update(
        {"grant_received": student.grant_received + new_grant_received},
        synchronize_session=False,
    )
    db.commit()
    return student
